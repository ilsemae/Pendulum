function EOMs_derive_branching_pendulum_lagrange() 
    ...
    r_E3_E1 = l3  * (sin(theta3)*i - cos(theta3)*j);
    r_G2_E1 = r_E2_E1/2;
    r_G3_E1 = r_E3_E1/2;
     
    v_G1_0 = cross(thetadot1*k, r_G1_0);
    v_G2_0 = cross(thetadot1*k, r_E1_0)  +  cross(thetadot2*k, r_G2_E1);
    v_G3_0 = cross(thetadot1*k, r_E1_0)  +  cross(thetadot3*k, r_G3_E1);
    ...
    PE_3   = m3*g*(l1*(1-cos(theta1))+l3/2*(1-cos(theta3)));
    ...
    rhs_file = fopen('branching_pendulum_lagrange_rhs.m','w');
    fprintf(rhs_file,'function qdot = branching_pendulum_lagrange_rhs(tspan,q0,flag,p) \n');
    fprintf(rhs_file,' \n');
    %unpacking
